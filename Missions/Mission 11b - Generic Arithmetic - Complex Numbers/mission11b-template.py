...
# Why it happens: The operators in the complex number package make use of the generic operators in the 
# generic number package to perform the arithmetic on the real and imaginary parts. Since the generic 
# operators call on apply_generic which requires the numbers to be tagged, first_try fails as 9 and 10 
# are untagged.
...
# Answer: add_com makes a call to the generic function 'add' from the generic number package, in 
# its return statement. If add_com were to be named add, it would cause it to call itself, which
